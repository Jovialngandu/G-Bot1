...
export async function handleTextMessage(ctx) {
    ...
    try {
        ...
        // Détection des erreurs spécifiques de l'API Gemini
        if (error.status === 429) {
            return ctx.reply("⚠️ L'IA est un peu trop sollicitée en ce moment (limite de requêtes atteinte). Réessayez dans quelques minutes !");
        }
        if (error.status === 503) {
            return ctx.reply("⏳ Les serveurs de Google sont surchargés suite à une forte demande. Réessayez d'ici un court instant.");
        }
